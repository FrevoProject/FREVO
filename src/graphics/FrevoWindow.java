...
public class FrevoWindow extends JFrame implements MouseListener,
	...
	private long activeSeed = FrevoMain.getSeed();
	...
	public FrevoWindow() {
		...
		// set initial size
		setBounds(0, 0, FrevoMain.mainWindowParameters[0],
		...
		UIManager.put("ProgressBar.selectionForeground", Color.WHITE.darker());
		...
		seedTextField.setText(Long.toString(FrevoMain.getSeed()));
		...
		seedresTextField.setText(Long.toString(FrevoMain.getSeed()));
	...
	public void updateAdvancedLabels() {
		...
		if (FrevoMain.getNumberOfSimulationRuns() > 1)
			text = FrevoMain.getNumberOfSimulationRuns() + " runs of <i>"
					...
					+ "</i><br> with starting seed of " + FrevoMain.getSeed();
		else
			text = FrevoMain.getNumberOfSimulationRuns() + " run of <i>"
					+ FrevoMain.getCustomName()
					+ "</i><br> with starting seed of " + FrevoMain.getSeed();

		advancedLabel.setText("<font face=\"ARIAL\" size=\"3\">" + text
				+ "</font>");
	...
	private class LoadingWorkerThread extends SwingWorker<Void, Float> {
		 ...
		 */
		private class RepresentationComparator implements
				Comparator<AbstractRepresentation> {
			@Override
			public int compare(AbstractRepresentation arg0,
					AbstractRepresentation arg1) {
	...
	public class SimulationWorkerThread extends SwingWorker<Void, Float> {
		...
		public void done() {
			...
			if (FrevoMain.getCurrentRun() < FrevoMain
				...
				simulationWorkerThread = new SimulationWorkerThread(
						statisticChartComponents, seed + 1);
				simulationWorkerThread.execute();
			} else {
	...
	public void actionPerformed(ActionEvent e) {
		...
		// go back
			...
			if (FrevoMain.isRunning) {
				...
				Document doc = FrevoMain.loadSession(loadedFile);
				FrevoMain.setNumberOfSimulationRuns(numberofrunsTextField
						.getIntegerText());
	...
	public void insertUpdate(DocumentEvent e) {
		if (e.getDocument() == simnameTextField.getDocument()) {
			...
			try {
				FrevoMain.setSeed(Long.parseLong(seedTextField.getText()));
			} catch (NumberFormatException e1) {
				FrevoMain.setSeed(12345);
			}
