...
/**
 * A not recursive N-layered neural network
 * 
 * @author Istvan
 */
public class FeedForwardNetwork extends AbstractRepresentation {
	...
	public FeedForwardNetwork(int inputnumber, int outputnumber,
		...
		XMLFieldEntry ln = getProperties().get("layers");
		layers = Integer.parseInt(ln.getValue());
		...
		// number of nodes = all nodes
		nodes = input_nodes + output_nodes + hidden_nodes*layers;

		// generate structure
		this.weight = new float[this.nodes][this.nodes]; // weight matrix
	...
	public void recombinationFunction(AbstractRepresentation other, int method) {
		if (!(other instanceof FeedForwardNetwork))
		...
		FeedForwardNetwork offspring = this;
		FeedForwardNetwork father = (FeedForwardNetwork) other;
	...
	public ArrayList<Float> getStep(ArrayList<Float> input) {
		...
		for (int hiddenNodeIndex = input_nodes; hiddenNodeIndex < input_nodes + hidden_nodes; hiddenNodeIndex++) { // all hidden														// nodes
			float sum = 0;
			for (int inputNodeIndex = 0; inputNodeIndex < input_nodes; inputNodeIndex++) // all input nodes
			{
				sum += this.weight[inputNodeIndex][hiddenNodeIndex] * this.output[inputNodeIndex];
			}
			this.activation[hiddenNodeIndex] = this.bias[hiddenNodeIndex] + sum;
			this.output[hiddenNodeIndex] = activate(this.activation[hiddenNodeIndex]);
		}
		
		// propagate to all subsequent hidden layers
		for (int layerIndex = 1; layerIndex < this.layers; layerIndex++) {
			int hiddenLayerIndex = input_nodes+(hidden_nodes*layerIndex); // index to the current hidden layer
			int prevHiddenLayerIndex = hiddenLayerIndex - hidden_nodes; // index to the previous hidden layer
			for (int hiddenNodeIndex = hiddenLayerIndex; hiddenNodeIndex < hiddenLayerIndex + hidden_nodes; hiddenNodeIndex++) {
				float sum = 0;
				for (int inputNodeIndex = prevHiddenLayerIndex; inputNodeIndex < prevHiddenLayerIndex+hidden_nodes; inputNodeIndex++) // all input nodes
				{
					sum += this.weight[inputNodeIndex][hiddenNodeIndex] * this.output[inputNodeIndex];
				}
				this.activation[hiddenNodeIndex] = this.bias[hiddenNodeIndex] + sum;
		...
		{
			float sum = 0;
			for (int j = nodes - output_nodes - hidden_nodes; j < nodes - output_nodes; j++) // last hidden layer nodes
			{
	...
	public double diffTo(AbstractRepresentation o) {
		...
		if (!(o instanceof FeedForwardNetwork))
			throw new IllegalArgumentException(
					"diffTo between different network classes not possible!");

		FeedForwardNetwork network = (FeedForwardNetwork) o;
	...
	public AbstractRepresentation cloneFunction() {
		FeedForwardNetwork res = new FeedForwardNetwork(input_nodes,
	...
	public Hashtable<String, String> getDetails() {
		...
		result.put("layers", Integer.toString(this.layers));
	...
	public void exportToFile(File saveFile) {
		// TODO: this function is copied straight from 3 layered network so it will not be accurate
